...
6. **AÇÕES FINANCEIRAS AUTOMÁTICAS** - MUITO IMPORTANTE:
   ...
   c) ATUALIZAR TRANSAÇÃO (quando o usuário quiser corrigir QUALQUER campo de um lançamento já feito):
   \`\`\`json:action
   [{"action": "update_transaction", "description": "parte_da_descricao_para_buscar", "newDescription": "nova_descricao_opcional", "amount": novo_valor, "type": "expense|income", "frequency": "once|monthly|weekly|yearly"}]
   \`\`\`
   - O campo "description" é usado para BUSCAR o lançamento existente (busca aproximada).
   - Inclua APENAS os campos que o usuário pediu para mudar. Os demais ficam como estão.
   - Use "newDescription" se o usuário quiser renomear o lançamento.
   ...
   e) ATUALIZAR META (quando o usuário quiser mudar QUALQUER campo de uma meta):
   \`\`\`json:action
   [{"action": "update_goal", "goalName": "parte_do_nome_para_buscar", "newName": "novo_nome_opcional", "targetAmount": novo_valor, "currentAmount": novo_valor_atual, "monthlyContribution": nova_contribuicao, "deadline": "YYYY-MM-DD"}]
   \`\`\`
   - Use "goalName" para BUSCAR a meta existente (busca aproximada).
   - Inclua APENAS os campos que mudaram. Use "newName" para renomear.
   - Se o usuário disser "já depositei R$ 500 na meta da viagem", some no currentAmount existente — informe que foi atualizado.
   ...
   - Para metas, use update_goal sempre que o usuário pedir alteração ("aumenta a meta para X", "já guardei Y na meta Z", "muda o prazo da meta W para...").
   - Pode incluir múltiplas ações no mesmo array.
   - Sempre informe ao usuário o que foi registrado/atualizado/excluído.
...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        // Modelo da Groq com bom equilíbrio de qualidade x velocidade.
        // Outros disponíveis: "llama-3.1-70b-versatile", "mixtral-8x7b-32768"
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return new Response(JSON.stringify({ error: "Chave da IA inválida ou expirada." }), {
          status: 401,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      const t = await response.text();
      console.error("Erro no provedor de IA:", response.status, t);
