...
// Tipo de uma ação financeira recebida da resposta do assistente.
...
function parseFinancialActions(content: string): FinancialAction[] {
...
// Executa as ações financeiras detectadas na resposta do assistente.
  ...
  for (const action of actions) {
    try {
      if (action.action === "create_transaction" && action.amount && action.description) {
        ...
        if (data && data.length > 0) {
          const updates: Record<string, unknown> = {};
          if (action.newName) updates.name = action.newName;
          if (action.targetAmount !== undefined) updates.target_amount = action.targetAmount;
          if (action.currentAmount !== undefined) updates.current_amount = action.currentAmount;
          if (action.monthlyContribution !== undefined) updates.monthly_contribution = action.monthlyContribution;
          if (action.deadline !== undefined) updates.deadline = action.deadline;
          if (Object.keys(updates).length > 0) {
            await supabase.from("goals").update(updates).eq("id", data[0].id);
          }
        }
      } else if (action.action === "delete_goal" && action.goalName) {
        const { data } = await supabase
          .from("goals")
          .select("id")
          .eq("user_id", userId)
